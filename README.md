...
- **Tested** — 153 passing tests (Vitest): backend unit + integration
...
└── tools/screenshots/     # Playwright harness that generates the assets above
```
...
The images in `docs/screenshots/` are generated by a Playwright harness in
`tools/screenshots/`. With the stack up, seeded, and a load test run:

```bash
cd tools/screenshots
npm install        # installs Playwright + a Chromium build
npm run shots      # writes PNGs + GIFs into docs/screenshots/
```

GIFs are assembled from frame sequences with a pure-JS encoder (`gifenc` +
`pngjs`) — no `ffmpeg` required.
